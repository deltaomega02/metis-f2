...
from config.settings import get_symbol_spec
...
def calculate_position_size(
    ...
    Args:
        ...
        leverage: 레버리지 배수
        entry_price: 진입 예정가
        symbol: 거래 symbol (qty precision + min_order_qty 결정)

    Returns:
        (position_value_usdt, quantity_in_base_coin)

    Note:
        - fee/slippage buffer 1.5% 차감 (SAFETY_BUFFER=0.985)
        - quantity는 *내림(floor)* 처리해 잔고 초과 방지
          (이전엔 round() 위로 반올림돼서 잔고 초과 → InsufficientBalanceError)
        - symbol별 qty precision 사용 (BTC 3자리, XRP 0자리)
    """
    SAFETY_BUFFER = 0.985  # 1.5% buffer (fees + slippage)
    margin = balance * SAFETY_BUFFER
    position_value = margin * leverage
    raw_quantity = position_value / entry_price

    spec = get_symbol_spec(symbol)
    precision = spec.get("qty_precision", TRADING.QTY_PRECISION)
    min_qty = spec.get("min_order_qty", TRADING.MIN_ORDER_QTY)

    # 핵심 fix: round → floor 로 잔고 초과 차단
    factor = 10 ** precision
    quantity = math.floor(raw_quantity * factor) / factor
...
def validate_ai_strategy(
    ...
    # 5. 포지션 사이즈 (symbol별 qty precision 사용)
    position_value, quantity = calculate_position_size(balance, leverage, current_price, symbol)
    ...
    if quantity < min_qty or quantity == 0:
        return {
            "valid": False,
            "reason": f"주문 수량({quantity} {base_coin})이 최소 단위({min_qty} {base_coin}) 미만"
        }
